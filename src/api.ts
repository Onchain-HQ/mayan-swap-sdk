...
export async function fetchTokenList(
	...
	const res = await fetch(
		`${addresses.PRICE_URL}/tokens?${toQueryString(queryParams)}`
	);
	await check5xxError(res);
	if (res.status === 200) {
		const result = await res.json();
...
export function generateFetchQuoteUrl(
	params: QuoteParams,
...
export async function getSuggestedRelayer(): Promise<string> {
	const res = await fetch(
		`${addresses.RELAYER_URL}/active-relayers?solanaProgram=${addresses.MAYAN_PROGRAM_ID}`,
		{
			method: 'GET',
			redirect: 'follow',
		}
...
export async function getSwapSolana(
	params: GetSolanaSwapParams
): Promise<SolanaClientSwap> {
...
export async function submitSwiftEvmSwap(
	params: SwiftEvmGasLessParams,
	signature: string
): Promise<void> {
...
export async function submitSwiftSolanaSwap(
	signedTx: string
