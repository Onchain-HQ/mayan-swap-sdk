...
function nativeAddressToHexString(address, wChainId) {
  if (wChainId === 1) {
    ...
    return (0, import_ethers.keccak256)(address);
...
function getAssociatedTokenAddress(mint, owner, allowOwnerOffCurve = false, programId = new import_web3.PublicKey(addresses_default.TOKEN_PROGRAM_ID), associatedTokenProgramId = new import_web3.PublicKey(
...
function getEvmMctpCreateOrderParams(quote, destinationAddress, referrerAddress, signerChainId) {
  ...
  const tokenOut = quote.toToken.contract === import_ethers2.ZeroAddress ? nativeAddressToHexString(
    import_web33.SystemProgram.programId.toString(),
    getWormholeChainIdByName("solana")
...
function getMctpFromEvmTxPayload(quote, destinationAddress, referrerAddress, signerChainId, permit, payload) {
  ...
  const forwarder = new import_ethers2.Contract(
...
function createAssociatedTokenAccountInstruction(payer, associatedToken, owner, mint, programId = new import_web34.PublicKey(addresses_default.TOKEN_PROGRAM_ID), associatedTokenProgramId = new import_web34.PublicKey(
  addresses_default.ASSOCIATED_TOKEN_PROGRAM_ID
)) {
  const keys = [
...
async function createInitializeRandomTokenAccountInstructions(connection, payer, mint, owner, keyPair, programId = new import_web34.PublicKey(addresses_default.TOKEN_PROGRAM_ID)) {
  ...
  instructions.push(
    import_web34.SystemProgram.createAccount({
      fromPubkey: payer,
  ...
  instructions.push(
    new import_web34.TransactionInstruction({
      ...
      programId,
      data: import_buffer.Buffer.from([1])
    })
  );
  return instructions;
}
...
function createApproveInstruction(account, delegate, owner, amount, programId = new import_web34.PublicKey(addresses_default.TOKEN_PROGRAM_ID)) {
  ...
  const data = import_buffer.Buffer.alloc(ApproveInstructionData.span);
...
function createSyncNativeInstruction(account) {
  ...
  const data = import_buffer.Buffer.alloc(SyncNativeInstructionData.span);
...
var CloseAccountInstructionData = (0, import_buffer_layout.struct)([(0, import_buffer_layout.u8)("instruction")]);
function createCloseAccountInstruction(account, destination, owner, programId = new import_web34.PublicKey(addresses_default.TOKEN_PROGRAM_ID)) {
  ...
  const data = import_buffer.Buffer.alloc(CloseAccountInstructionData.span);
...
function createSplTransferInstruction(source, destination, owner, amount, programId = new import_web34.PublicKey(addresses_default.TOKEN_PROGRAM_ID)) {
  ...
  const data = import_buffer.Buffer.alloc(SplTransferInstructionData.span);
...
var solMint = new import_web34.PublicKey(
...
function deserializeInstructionInfo(instruction) {
  return new import_web34.TransactionInstruction({
    ...
    data: import_buffer.Buffer.from(instruction.data, "base64")
  });
}
...
function getAnchorInstructionData(name) {
  ...
  return import_buffer.Buffer.from((0, import_ethers3.sha256)((0, import_ethers3.toUtf8Bytes)(preimage))).subarray(0, 8);
}
...
async function sendJitoBundle(singedTrxs, options, forceToBeSubmitted = false) {
  try {
    ...
    const bundle = {
      ...
      id: 1,
      method: "sendBundle",
      params: [signedTrxs.map((trx) => (0, import_ethers3.encodeBase58)(trx))]
    };
    const res = await fetch(
...
function validateJupCleanupInstruction(instruction) {
  ...
  if (!instruction.programId.equals(new import_web34.PublicKey(addresses_default.TOKEN_PROGRAM_ID)) && !instruction.programId.equals(
    new import_web34.PublicKey(addresses_default.TOKEN_2022_PROGRAM_ID)
  )) {
...
function validateJupSetupInstructions(instructions, owner) {
  ...
  instructions.forEach((instruction) => {
    if (!instruction.programId.equals(
      new import_web34.PublicKey(addresses_default.ASSOCIATED_TOKEN_PROGRAM_ID)
    ) && !instruction.programId.equals(import_web34.SystemProgram.programId) && !instruction.programId.equals(
      new import_web34.PublicKey(addresses_default.TOKEN_PROGRAM_ID)
...
function validateJupSwapInstruction(instruction, validDestAccount) {
  if (!instruction.programId.equals(
    new import_web34.PublicKey("JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4")
  )) {
...
function createMctpBridgeWithFeeInstruction(ledger, toChain, mintAddress, relayerAddress, feeSolana) {
  ...
  const data = import_buffer2.Buffer.alloc(MCTPBridgeWithFeeLayout.span);
...
function createMctpBridgeLockFeeInstruction(ledger, toChain, mintAddress, relayerAddress, feeSolana) {
  ...
  const [feeState] = import_web36.PublicKey.findProgramAddressSync(
    [import_buffer2.Buffer.from("LOCKED_FEE"), mint.toBuffer(), cctpMessage.publicKey.toBytes()],
    mctpProgram
  );
  let relayerAccount;
  if (feeSolana && feeSolana > BigInt(0)) {
  ...
  const data = import_buffer2.Buffer.alloc(MctpBridgeLockFeeLayout.span);
...
function createMctpInitSwapInstruction(ledger, toChain, mintAddress, relayerAddress, feeSolana) {
  ...
  const [swapState] = import_web36.PublicKey.findProgramAddressSync(
    [import_buffer2.Buffer.from("ORDER_SOLANA_SOURCE"), ledger.toBuffer()],
    mctpProgram
  );
  let relayerAccount;
  if (feeSolana && feeSolana > BigInt(0)) {
  ...
  const data = import_buffer2.Buffer.alloc(MctpInitSwapLayout.span);
...
function createMctpBridgeLedgerInstruction(params) {
  ...
  const destAddress = import_buffer2.Buffer.from(
    hexToUint8Array(
  ...
  const refAddress = params.referrerAddress ? import_buffer2.Buffer.from(hexToUint8Array(
  ...
  const data = import_buffer2.Buffer.alloc(MctpBridgeLedgerLayout.span);
...
function createMctpSwapLedgerInstruction(params) {
  ...
  const feeSolana = getSafeU64Blob(params.feeSolana);
  const tokenOut = import_buffer2.Buffer.from(hexToUint8Array(
  ...
  const data = import_buffer2.Buffer.alloc(MctpSwapLedgerLayout.span);
...
async function createMctpFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}) {
  ...
  const [ledger] = import_web36.PublicKey.findProgramAddressSync(
    [import_buffer2.Buffer.from(ledgerSeedPrefix), user.toBytes(), randomKey.publicKey.toBytes()],
    mctpProgram
  );
  const ledgerAccount = getAssociatedTokenAddress(
...
function createSwiftOrderHash(quote, swapperAddress, destinationAddress, referrerAddress, randomKeyHex) {
  ...
  const data = import_buffer3.Buffer.alloc(orderDataSize);
  let offset = 0;
  const sourceChainId = getWormholeChainIdByName(quote.fromChain);
  const trader = import_buffer3.Buffer.from(hexToUint8Array(nativeAddressToHexString(swapperAddress, sourceChainId)));
  data.set(trader, 0);
  offset += 32;
  data.writeUInt16BE(sourceChainId, offset);
  offset += 2;
  const _tokenIn = quote.swiftInputContract === import_ethers4.ZeroAddress ? nativeAddressToHexString(import_web37.SystemProgram.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(quote.swiftInputContract, sourceChainId);
  const tokenIn = import_buffer3.Buffer.from(hexToUint8Array(_tokenIn));
  data.set(tokenIn, offset);
  offset += 32;
  const destinationChainId = getWormholeChainIdByName(quote.toChain);
  const destAddress = import_buffer3.Buffer.from(hexToUint8Array(nativeAddressToHexString(destinationAddress, destinationChainId)));
  data.set(destAddress, offset);
  offset += 32;
  data.writeUInt16BE(destinationChainId, offset);
  offset += 2;
  const _tokenOut = quote.toToken.contract === import_ethers4.ZeroAddress ? nativeAddressToHexString(import_web37.SystemProgram.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(quote.toToken.contract, destinationChainId);
  const tokenOut = import_buffer3.Buffer.from(hexToUint8Array(_tokenOut));
  ...
  offset += 8;
  const refAddress = referrerAddress ? import_buffer3.Buffer.from(hexToUint8Array(nativeAddressToHexString(referrerAddress, destinationChainId))) : import_web37.SystemProgram.programId.toBuffer();
  ...
  offset += 1;
  const _randomKey = import_buffer3.Buffer.from(hexToUint8Array(randomKeyHex));
  data.set(_randomKey, offset);
  offset += 32;
  if (offset !== orderDataSize) {
    throw new Error(`Invalid order data size: ${offset}`);
  }
  const hash = import_ethers4.ethers.keccak256(data);
  return import_buffer3.Buffer.from(hexToUint8Array(hash));
}
...
function createSwiftInitInstruction(params) {
  const { quote } = params;
  const mint = quote.swiftInputContract === import_ethers4.ZeroAddress ? solMint : new import_web37.PublicKey(quote.swiftInputContract);
  const destinationChainId = getWormholeChainIdByName(quote.toChain);
  ...
  const data = import_buffer3.Buffer.alloc(InitSwiftLayout.span);
  const refAddress = params.referrerAddress ? import_buffer3.Buffer.from(hexToUint8Array(nativeAddressToHexString(params.referrerAddress, destinationChainId))) : import_web37.SystemProgram.programId.toBuffer();
  ...
  InitSwiftLayout.encode({
    ...
    nativeInput: quote.swiftInputContract === import_ethers4.ZeroAddress ? 1 : 0,
    feeSubmit: getSafeU64Blob(BigInt(quote.submitRelayerFee64)),
    destAddress: import_buffer3.Buffer.from(hexToUint8Array(nativeAddressToHexString(params.destinationAddress, destinationChainId))),
    destinationChain: destinationChainId,
    tokenOut: import_buffer3.Buffer.from(hexToUint8Array(nativeAddressToHexString(quote.toToken.contract, destinationChainId))),
...
async function createSwiftFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}) {
  ...
  const [state] = import_web37.PublicKey.findProgramAddressSync(
    [import_buffer3.Buffer.from("STATE_SOURCE"), hash],
    swiftProgram
  );
  const swiftInputMint = quote.swiftInputContract === import_ethers4.ZeroAddress ? solMint : new import_web37.PublicKey(quote.swiftInputContract);
  ...
  if (quote.fromToken.contract === quote.swiftInputContract) {
    ...
    if (quote.swiftInputContract === import_ethers4.ZeroAddress) {
      instructions.push(
...
async function createSwapFromSolanaInstructions(quote, swapperWalletAddress, destinationAddress, referrerAddresses, connection, options = {}) {
  ...
  if (quote.suggestedPriorityFee > 0) {
    instructions.push(
      import_web38.ComputeBudgetProgram.setComputeUnitPrice({
        microLamports: quote.suggestedPriorityFee
      })
    );
  }
  ...
  const [mayanFee, mayanFeeNonce] = import_web38.PublicKey.findProgramAddressSync(
    [import_buffer4.Buffer.from("MAYANFEE")],
    mayanProgram
  );
  const [referrerFee, referrerFeeNonce] = import_web38.PublicKey.findProgramAddressSync(
    [import_buffer4.Buffer.from("REFERRERFEE"), referrerAddr.toBuffer()],
    mayanProgram
  );
  ...
  const [state, stateNonce] = import_web38.PublicKey.findProgramAddressSync(
    [
      import_buffer4.Buffer.from("V2STATE"),
      import_buffer4.Buffer.from(msg1.publicKey.toBytes()),
      import_buffer4.Buffer.from(msg2.publicKey.toBytes())
    ],
    mayanProgram
  );
  ...
  if (destinationChainId === 1) {
    const destinationAccount = await solanaConnection.getAccountInfo(
      new import_web38.PublicKey(destinationAddress)
    );
  ...
  const destAddress = import_buffer4.Buffer.from(
    hexToUint8Array(
  ...
  const deadline = BigInt(quote.deadline64);
  const swapData = import_buffer4.Buffer.alloc(SwapLayout.span);
  const swapFields = {
    instruction: 101,
    stateNonce,
    amount: getSafeU64Blob(amount),
...
async function swapFromSolana(quote, swapperWalletAddress, destinationAddress, referrerAddresses, signTransaction, connection, extraRpcs, sendOptions, jitoOptions, instructionOptions) {
  ...
  if (jitoEnabled) {
    ...
    if (signedTrx instanceof import_web38.Transaction && signedTrx?.signatures[0]?.publicKey) {
      mayanTxHash = (0, import_ethers5.encodeBase58)(
      ...
      mayanTxHash = (0, import_ethers5.encodeBase58)(Uint8Array.from(signedTrx.signatures[0]));
    }
    if (mayanTxHash === null) {
  ...
  if (quote.gasless) {
    const serializedTrx = import_buffer4.Buffer.from(signedTrx.serialize()).toString("base64");
...
function getEvmSwiftParams(quote, swapperAddress, destinationAddress, referrerAddress, signerChainId) {
  ...
  const tokenOut = quote.toToken.contract === import_ethers6.ZeroAddress ? nativeAddressToHexString(import_web39.SystemProgram.programId.toString(), 1) : nativeAddressToHexString(quote.toToken.contract, destChainId);
  const minAmountOut = getAmountOfFractionalAmount(
    quote.minAmountOut,
    Math.min(8, quote.toToken.decimals)
  );
...
function getSwiftFromEvmTxPayload(quote, swapperAddress, destinationAddress, referrerAddress, signerChainId, permit) {
  ...
  let swiftCallData;
  const swiftContract = new import_ethers6.Contract(swiftContractAddress, MayanSwiftArtifact_default.abi);
...
function getShuttleParams(quote, destinationAddress, signerChainId) {
  ...
  if (shuttleParams.hasDestSwap) {
    if (quote.toToken.contract === import_ethers7.ZeroAddress) {
...
function getShuttleFromEvmTxPayload(quote, destinationAddress, signerChainId, permit) {
  ...
  signerChainId = Number(signerChainId);
  const _permit = permit || ZeroPermit;
  const forwarder = new import_ethers7.Contract(
    addresses_default.MAYAN_FORWARDER_CONTRACT,
    MayanForwarderArtifact_default.abi
  );
  const {
  ...
  let shuttleCallData;
  const shuttleContract = new import_ethers7.Contract(
...
function getEvmFastMctpBridgeTxPayload(quote, destinationAddress, referrerAddress, signerChainId, payload) {
  ...
  const fastMctpContract = new import_ethers8.Contract(
    contractAddress,
...
function getEvmFastMctpCreateOrderParams(quote, destinationAddress, referrerAddress, signerChainId) {
  ...
  const deadline = BigInt(quote.deadline64);
  const tokenOut = quote.toToken.contract === import_ethers8.ZeroAddress ? nativeAddressToHexString(
    import_web310.SystemProgram.programId.toString(),
    getWormholeChainIdByName("solana")
  ) : nativeAddressToHexString(
    quote.toChain === "sui" ? quote.toToken.verifiedAddress : quote.toToken.contract,
...
function getEvmFastMctpCreateOrderTxPayload(quote, destinationAddress, referrerAddress, signerChainId) {
  ...
  const value = (0, import_ethers8.toBeHex)(0);
  return {
    to: contractAddress,
    data,
    value,
...
function getFastMctpFromEvmTxPayload(quote, destinationAddress, referrerAddress, signerChainId, permit, payload) {
  ...
  signerChainId = Number(signerChainId);
  const _permit = permit || ZeroPermit;
  const forwarder = new import_ethers8.Contract(
    addresses_default.MAYAN_FORWARDER_CONTRACT,
    MayanForwarderArtifact_default.abi
  );
    ...
    if (quote.hasAuction) {
      ...
      if (quote.fromToken.contract === import_ethers8.ZeroAddress) {
        ...
        return {
          data,
          to: addresses_default.MAYAN_FORWARDER_CONTRACT,
          value: (0, import_ethers8.toBeHex)(fastMctpPayloadIx._params.amountIn),
          chainId: signerChainId,
          _forwarder: {
            method: forwarderMethod,
            params: forwarderParams
...
function getEvmSwapParams(quote, destinationAddress, referrerAddress, signerAddress, signerChainId, payload) {
  ...
  const unwrapRedeem = quote.toToken.contract === import_ethers9.ZeroAddress;
  const criteria = {
...
function getSwapFromEvmTxPayload(quote, swapperAddress, destinationAddress, referrerAddresses, signerAddress, signerChainId, payload, permit) {
  ...
  const forwarderContract = new import_ethers9.Contract(
    addresses_default.MAYAN_FORWARDER_CONTRACT,
    MayanForwarderArtifact_default.abi
  );
  const mayanSwap = new import_ethers9.Contract(contractAddress, MayanSwapArtifact_default.abi);
  let forwarderMethod;
  let forwarderParams;
  let value;
  const _permit = permit || ZeroPermit;
  if (tokenIn === import_ethers9.ZeroAddress) {
    ...
    value = (0, import_ethers9.toBeHex)(amountIn);
  } else {
    ...
    value = (0, import_ethers9.toBeHex)(bridgeFee);
  }
